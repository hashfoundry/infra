# 58. How do you handle ConfigMap and Secret versioning?

## 🎯 **Управление версиями ConfigMaps и Secrets**

**Версионирование ConfigMaps и Secrets** - это критически важная практика для обеспечения надежности, отслеживаемости и возможности отката изменений в Kubernetes. Правильное управление версиями помогает избежать проблем при обновлениях и обеспечивает стабильность приложений.

## 🏗️ **Основные стратегии версионирования:**

### **1. Именование с версиями (Versioned Naming)**
### **2. Использование labels и annotations**
### **3. Immutable ConfigMaps и Secrets**
### **4. GitOps подход с версионированием**
### **5. Автоматизированное управление версиями**

## 📊 **Практические примеры из вашего HA кластера:**

### **1. Создание демонстрационной среды:**
```bash
# Создать namespace для демонстрации версионирования
kubectl create namespace config-versioning-demo

# Создать начальную версию ConfigMap с версионированным именем
...
# Добавить labels и annotations для версионирования
...
# Создать начальную версию Secret
...
# Добавить labels и annotations для Secret
...
### **2. Создание новых версий с улучшенным именованием:**
```bash
# Создать версию v2 ConfigMap с новыми настройками
...
# Создать версию v3 для production
...
### **3. Демонстрация использования версионированных конфигураций:**
```bash
# Создать Deployment, использующий версию v1
...
# Создать Services для каждой версии
cat << EOF | kubectl apply -f -
apiVersion: v1
kind: Service
metadata:
...
### **4. Создание системы управления версиями:**
```bash
# Создать скрипт для управления версиями конфигураций
cat << 'EOF' > config-version-manager.sh
#!/bin/bash

NAMESPACE="config-versioning-demo"

# Функция для отображения всех версий
list_versions() {
...
# Функция для получения детальной информации о версии
describe_version() {
    local version=$1
...
compare_versions() {
    ...
    # Сравнить Secrets (только ключи, не значения)
...
# Функция для создания новой версии на основе существующей
create_new_version() {
    ...
    # Проверить, что базовая версия существует
    ...
    # Создать новый ConfigMap на основе существующего
    ...
    # Создать новый Secret на основе существующего
    ...
    # Обновить labels и annotations для Secret
    ...
    echo "✓ Configuration version $new_version created successfully"
}
...
rollback_deployment() {
    ...
    # Проверить, что целевая версия существует
    ...
    # Обновить Deployment для использования новой версии
...
# Запустить основную функцию с переданными аргументами
main "$@"
...
# Демонстрация использования скрипта
...
# Попытаться изменить immutable ConfigMap (это должно завершиться ошибкой)
echo "=== Testing Immutable ConfigMap Protection ==="
kubectl patch configmap app-config-immutable-v1 -n config-versioning-demo --type merge -p='{"data":{"app.version":"2.0.0"}}' || echo "✓ Immutable ConfigMap correctly protected from changes"

# Попытаться изменить immutable Secret (это должно завершиться ошибкой)
echo "=== Testing Immutable Secret Protection ==="
kubectl patch secret app-secrets-immutable-v1 -n config-versioning-demo --type merge -p='{"data":{"api.key":"'$(echo -n "new_api_key" | base64)'"}}' || echo "✓ Immutable Secret correctly protected from changes"

# Показать, что для изменения нужно создать новую версию
...
data:
  ...
  backup.enabled: "true"
  monitoring.enabled: "true"
  config.checksum: "sha256:def456ghi789jkl012"  # Новый checksum
EOF

# Проверить immutable ресурсы
kubectl get configmaps,secrets -n config-versioning-demo -l config.hashfoundry.io/immutable=true
```

### **6. GitOps подход с версионированием:**
```bash
# Создать структуру для GitOps версионирования
mkdir -p gitops-config/{environments,versions,templates}

# Создать template для ConfigMap
...
# Создать конфигурацию для staging
...
# Создать скрипт для генерации версионированных конфигураций
...
generate_config() {
    ...
    # Загрузить переменные из environment файла
...
# Удалить все ConfigMaps и Secrets
...
## 📋 **Сводка стратегий версионирования:**

### **Основные команды:**
```bash
# Создание версионированного ConfigMap
kubectl create configmap app-config-v1 --from-literal=key=value

# Добавление версионных labels
...
## 📊 **Сравнительная таблица стратегий версионирования:**

| **Стратегия** | **Преимущества** | **Недостатки** | **Использование** |
|---------------|------------------|----------------|-------------------|
| **Versioned Naming** | Простота, ясность | Много ресурсов | Критичные конфигурации |
| **Labels/Annotations** | Метаданные, поиск | Дополнительная сложность | Отслеживание изменений |
| **Immutable Resources** | Безопасность, стабильность | Нужны новые ресурсы | Production окружения |
| **GitOps** | Автоматизация, аудит | Сложность настройки | CI/CD пайплайны |

## 🎯 **Best Practices для версионирования:**

### **1. Стратегия именования:**
- **Используйте семантическое версионирование** (v1.0.0, v1.1.0, v2.0.0)
- **Включайте окружение в имя** для ясности
- **Используйте consistent префиксы** для группировки
- **Документируйте схему именования**

### **2. Метаданные:**
- **Используйте стандартные labels** (app.kubernetes.io/*)
- **Добавляйте custom labels** для версионирования
- **Включайте changelog в annotations**
- **Отслеживайте git commits**

### **3. Управление жизненным циклом:**
- **Используйте immutable ресурсы** для production
- **Автоматизируйте создание версий**
- **Реализуйте стратегию очистки** старых версий
- **Мониторьте использование ресурсов**

### **4. Безопасность и аудит:**
- **Ограничивайте права на создание** версий
- **Логируйте все изменения**
- **Используйте approval процессы** для production
- **Регулярно аудируйте** версии конфигураций

**Правильное версионирование ConfigMaps и Secrets обеспечивает надежность, отслеживаемость и возможность быстрого отката в Kubernetes!**
